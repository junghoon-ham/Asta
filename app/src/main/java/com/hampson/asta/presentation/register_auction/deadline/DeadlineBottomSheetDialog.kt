...
@AndroidEntryPoint
class DeadlineBottomSheetDialog : BottomSheetDialogFragment() {
    ...
    private fun bindTitle() {
        binding.textViewTitle.text = getString(R.string.deadline_date)
    }

    private fun setupRecyclerView() {
        binding.recyclerView.apply {
            adapter = DeadlineAdapter().apply {
                context = this@DeadlineBottomSheetDialog.context
            }
            layoutManager = LinearLayoutManager(context, RecyclerView.VERTICAL, false)

            (adapter as DeadlineAdapter).setOnItemClickListener {
